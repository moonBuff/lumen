...
Lumen 就是包在模型外面的控制循环：负责组 prompt、解析模型输出、
...
class Lumen:
    def __init__(
        ...
        self.run_store = run_store or RunStore(Path(workspace.repo_root) / ".lumen" / "runs")
    ...
    def parse(raw):
        ...
        if "<tool>" in raw and ("<final>" not in raw or raw.find("<tool>") < raw.find("<final>")):
            body = Lumen.extract(raw, "tool")
            try:
                payload = json.loads(body)
            except Exception:
                return "retry", Lumen.retry_notice("model returned malformed tool JSON")
            if not isinstance(payload, dict):
                return "retry", Lumen.retry_notice("tool payload must be a JSON object")
            if not str(payload.get("name", "")).strip():
                return "retry", Lumen.retry_notice("tool payload is missing a tool name")
            args = payload.get("args", {})
            if args is None:
                payload["args"] = {}
            elif not isinstance(args, dict):
                return "retry", Lumen.retry_notice()
            return "tool", payload
        if "<tool" in raw and ("<final>" not in raw or raw.find("<tool") < raw.find("<final>")):
            payload = Lumen.parse_xml_tool(raw)
            if payload is not None:
                return "tool", payload
            return "retry", Lumen.retry_notice()
        if "<final>" in raw:
            final = Lumen.extract(raw, "final").strip()
            if final:
                return "final", final
            return "retry", Lumen.retry_notice("model returned an empty <final> answer")
        raw = raw.strip()
        if raw:
            return "final", raw
        return "retry", Lumen.retry_notice("model returned an empty response")
    ...
    def parse_xml_tool(raw):
        ...
        if not match:
            return None
        attrs = Lumen.parse_attrs(match.group("attrs"))
        ...
        for key in ("content", "old_text", "new_text", "command", "task", "pattern", "path"):
            if f"<{key}>" in body:
                args[key] = Lumen.extract_raw(body, key)
